...
uint64_t get_micro_time()
{
    ...
    t = ((tm.tv_sec * 1e6) + (tm.tv_nsec / 1e3));
    return t;
}
...
inet_to_tun(struct tundev *tdev, int index) {
	...
	char buf[BUFF_SIZE], outbuff[BUFF_SIZE], kcpbuf[BUFF_SIZE];
	ssize_t n, rn, kn;
    ...
    //ikcp recv
    pthread_mutex_lock(&tdev->mutex);
    ikcp_input(tdev->ptr_kcp, buf, n);
    ikcp_update(tdev->ptr_kcp, iclock());
    pthread_mutex_unlock(&tdev->mutex);

/*
	rn = mptun_decrypt(kcpbuf, kn, outbuff, tdev->key, tdev->ti);
...
*/
    while (1) {
        pthread_mutex_lock(&tdev->mutex);
        kn = ikcp_recv(tdev->ptr_kcp, kcpbuf, BUFF_SIZE);
        pthread_mutex_unlock(&tdev->mutex);
        fprintf(stderr, "kcp recv size %d\n", kn);
        if (kn > 0) {
        	for (;;) {
		        //int ret = tun_write(tunfd, outbuff, rn);
        		int ret = tun_write(tunfd, kcpbuf, kn);
		        if (ret < 0) {
        			if (errno == EINTR) {
		        		continue;
			        }
...
int udp_output(const char *buf, int len, ikcpcb *kcp, void *user){
    struct tundev *tdev = (struct tundev *)user;
	int tunfd = tdev->tunfd;
	...
	for (;;) {
		int ret = sendto(inetfd, buf, len, 0, (struct sockaddr *)addr, sizeof(SOCKADDR));
        fprintf(stderr, "udp_output sendto ret %d , len %d \n", ret, len);
...
/*
    ...
    */
    ...
    //ikcp_send(tdev->ptr_kcp, outbuf, n);
    pthread_mutex_lock(&tdev->mutex);
    ikcp_send(tdev->ptr_kcp, buf, n);
    ikcp_update(tdev->ptr_kcp, iclock());
    pthread_mutex_unlock(&tdev->mutex);
...
//static void
void* start(void* lp) {
    struct tundev *tdev = (struct tundev *)lp;
	struct sigaction sa;
	int i;
...
static void
ifconfig(const char * ifname, const char * va, const char *pa, int mtu) {
	char cmd[1024];
#if defined(__APPLE__)
	snprintf(cmd, sizeof(cmd), "ifconfig %s %s %s mtu 1380 netmask 255.255.255.255 up",
		ifname, va, pa);
#else
	snprintf(cmd, sizeof(cmd), "ifconfig %s %s netmask 255.255.255.255 pointopoint %s mtu %d",
		ifname, va, pa, mtu);
#endif
	if (system(cmd) < 0) {
		perror(cmd);
...
void* update_kcp(void* lp){
    ...
    while (1) {
        ...
        if (t1 >= t2){
            pthread_mutex_lock(&tdev->mutex);
            ikcp_update(tdev->ptr_kcp, t1);
            t2 = ikcp_check(tdev->ptr_kcp, t1);
            pthread_mutex_unlock(&tdev->mutex);
        }
        if (t2 - t1 > 0) {
            usleep(t2 - t1);
...
main(int argc, char *argv[]) {
    ...
    pthread_t update_thd, start_thd;
    int res=0;
    int mtu=1400;
	...
	ifconfig(ifname, vpnaddress, ptpaddress, mtu);
    ...
    //setup kcp
    tdev.ptr_kcp = ikcp_create(0x1234, &tdev);
    fprintf(stderr,"setup kcp with key %lld \n", tdev.key);

    if (!tdev.ptr_kcp) {
        return -1;
    }
    ikcp_nodelay(tdev.ptr_kcp, 1, 10, 2, 1);
    ikcp_wndsize(tdev.ptr_kcp, 128, 128);
    tdev.ptr_kcp->rx_minrto = 10;
    tdev.ptr_kcp->fastresend = 1;
    //ikcp_nodelay(tdev.ptr_kcp, 0, 40, 0, 0);
    ikcp_setmtu(tdev.ptr_kcp, mtu);
    tdev.ptr_kcp->output = udp_output;

    //pthread
    res = pthread_mutex_init(&tdev.mutex, NULL);
    if (res) {
        printf("pthread_mutex_init fail,error:%s\n", strerror(errno));
        return -1;
    }

    res = pthread_create(&update_thd, NULL, update_kcp, &tdev);
    if (res) {
        printf("pthread_create fail,error:%s\n", strerror(errno));
        return -1;
    }

    res = pthread_create(&update_thd, NULL, start, &tdev);
    if (res) {
        printf("pthread_create fail,error:%s\n", strerror(errno));
        return -1;
    }
    pthread_join(update_thd, NULL);
    pthread_join(start_thd, NULL);

    ikcp_release(tdev.ptr_kcp);
